...
async function installDependencies(dependencies, pluginName, plgPath) {
    ...
    if (notInstalled.length > 0) logger.custom(`Installing dependencies for ${fileName}`, ' PARADOX - LOADING ', '\x1b[33m');
    else return true;

    for (const dependency of notInstalled) {
        try {
            execSync(`npm install --package-lock false ${dependency}`, { stdio: 'ignore' });
            logger.custom(`Installed dependency ${dependency}`, ' PARADOX - LOADING ', '\x1b[32m');
            installed.push(dependency);
        } catch (err) {
            logger.custom(`Can't install dependency ${dependency}`, ' PARADOX - LOADING ', '\x1b[31m');
            logger.custom(err.message || err, 'PARADOX - LOADING ', '\x1b[31m');
    ...
    if (failed) {
        logger.custom('Install failed, reversing...', ' PARADOX - LOADING ', '\x1b[33m');
        execSync(`npm uninstall --package-lock false ${installed.join(' ')}`, { stdio: 'ignore' });

        logger.custom('Reversed dependencies installation!', ' PARADOX - LOADING ', '\x1b[32m');
...
async function loadPlugins() {
    logger.custom('Tiến hành đọc thuộc tính của plugins...', ' PARADOX - LOADING ', '\x1b[33m');
    const pluginsPath = resolvePath(process.cwd(), 'plugins');
    const plugins = ["commands", "events", "customs", "onMessage"];

    for (const plugin of plugins) {
        logger.custom(`Tiến hành đọc thuộc tính của ${plugin}...`, ' PARADOX - LOADING ', '\x1b[33m');
